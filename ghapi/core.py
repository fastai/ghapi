...
# %% auto 0
__all__ = ['GH_HOST', 'EMPTY_TREE_SHA', 'print_summary', 'GhApi', 'date2gh', 'gh2date', 'TokenNotAuthorized']
...
# %% ../00_core.ipynb 117
class TokenNotAuthorized(Exception): pass
...
def enable_pages(self:GhApi, branch=None, path="/"):
    ...
    try: r = self.repos.get()
    except HTTPError: raise TokenNotAuthorized(f'Not able to access repo with supplied Token')
    else:
        branch = branch or r.default_branch
        source = {"branch": branch, "path": path}
        if r.has_pages: return 'has_pages'# self.repos.update_information_about_pages_site(source=source)
